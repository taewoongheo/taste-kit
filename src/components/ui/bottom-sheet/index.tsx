...
const BottomSheetComponent = forwardRef<BottomSheetMethods, BottomSheetProps>(
  (
    ...
    const maxSnapPoint = useMemo<number>(() => Math.max(...parsedSnapPoints), [parsedSnapPoints]);

    const minSnapPoint = useMemo<number>(() => Math.min(...parsedSnapPoints), [parsedSnapPoints]);

    const maxSnapIndex = useMemo<number>(() => parsedSnapPoints.length - 1, [parsedSnapPoints]);
...
export const BottomSheet =
  memo<React.ForwardRefExoticComponent<BottomSheetProps & React.RefAttributes<BottomSheetMethods>>>(
