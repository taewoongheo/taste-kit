...
export const SearchBar = ({
  ...
  const animatedContainerStyle = useAnimatedStyle(() => {
    ...
    const searchBarWidth = interpolate(
      focusProgress.value,
      [0, 1],
      [currentWidth.value, focusedWidth || currentWidth.value - cancelButtonWidth],
    );
  ...
  const animatedSearchContentStyle = useAnimatedStyle(() => {
    const justifyContent =
      focusProgress.value === 0 && centerWhenUnfocused ? 'center' : 'flex-start';
  ...
  const animatedInputWrapperStyle = useAnimatedStyle(() => {
    ...
    const translateX = interpolate(focusProgress.value, [0, 1], [centerOffset, 0], {
  ...
  return (
    <View style={[styles.container, style]} onLayout={handleLayout}>
      <View style={styles.searchRow}>
        <AnimatedView style={[styles.searchBarContainer, animatedContainerStyle]}>
          <BlurView intensity={15} tint="systemChromeMaterialDark" style={styles.blurContainer}>
            <View style={styles.searchContainer}>
              <AnimatedView style={[styles.searchContent, animatedSearchContentStyle]}>
                <AnimatedView
                  style={[styles.searchIconContainer, animatedIconStyle, props?.iconStyle]}
                >
                  {renderLeadingIcons ? (
                    renderLeadingIcons()
                  ) : (
                    <SymbolView
                      ...
                      size={18}
                      tintColor="#8E8E93"
                      fallback={<Ionicons name="search" size={18} color="#8E8E93" />}
                    />
                  )}
                </AnimatedView>
