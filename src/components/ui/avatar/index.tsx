...
export const Avatar: React.FC<IAvatar> & React.FunctionComponent<IAvatar> = memo<IAvatar>(
  ({
    image,
    size = 40,
    onPress,
  ...
  }: IAvatar): (React.ReactNode & React.JSX.Element & React.ReactElement) | null => {
    const [_hasError, setHasError] = useState<boolean>(false);
    ...
    const shimmerAnimatedStylez = useAnimatedStyle<Pick<ViewStyle, 'opacity' | 'transform'>>(() => {
    ...
    const pressAnimatedStylez = useAnimatedStyle<Pick<ViewStyle, 'opacity' | 'transform'>>(() => ({
    ...
    const AvatarContent: React.FC = (): React.ReactNode & React.ReactElement => {
      if (loading) {
        return (
...
export default memo<React.FC<IAvatar> & React.FunctionComponent<IAvatar>>(Avatar);
